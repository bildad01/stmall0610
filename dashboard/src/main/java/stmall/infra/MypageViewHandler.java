...
public class MypageViewHandler {
    ...
    public void whenOrderPlaced_then_CREATE_1(
        ...
        try {
            ...
            mypage.setAdress(orderPlaced.getAddress());
    ...
    public void whenDeliveryCompleted_then_UPDATE_1(
        ...
        try {
            ...
            if (mypageOptional.isPresent()) {
                ...
                mypage.setDelivertStatus(DELIVER);
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
            }
        } catch (Exception e) {
    ...
    public void whenOrderCancelled_then_UPDATE_2(
        ...
        try {
            ...
            if (mypageOptional.isPresent()) {
                Mypage mypage = mypageOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                mypage.setOrderStatus(ORDDERCANCELLED);
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
            }
        } catch (Exception e) {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryReterned_then_UPDATE_3(
        @Payload DeliveryReterned deliveryReterned
    ) {
        try {
            if (!deliveryReterned.validate()) return;
            // view 객체 조회
            Optional<Mypage> mypageOptional = mypageRepository.findByOrderId(
                deliveryReterned.getOrderId()
            );

            if (mypageOptional.isPresent()) {
                Mypage mypage = mypageOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                mypage.setDelivertStatus(RETURNED);
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
            }
        } catch (Exception e) {
